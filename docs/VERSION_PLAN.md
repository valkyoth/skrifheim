...
- split audit transcript contract with non-circular transcript types:
  ...
  proof roots,
- canonical protected-read state machine:
  `AuditIntent -> ReadReleasePermit -> ReleaseStarted -> BatchMayRelease* ->
  ReadOutcomeReceipt`, with batch markers proving only the maximum data that
  may have been released, not client receipt,
