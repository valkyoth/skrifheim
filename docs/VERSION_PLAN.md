...
- split audit transcript contract with non-circular transcript types:
  ...
  general `OperationIntentId`, predecessor manifest root, and protected metadata
  before commit; `WriteCommitReceipt`, binding the intent digest to the
  completed WAL commit root, resulting world revision, prior audit record,
  output manifest generation, and mutating-operation `TransactionId` after
  commit; `ReadReleasePermit`, binding the intent digest to snapshot root,
  query digest, authorized-plan digest, result security label,
...
- protected-read verifier rules: denied outcomes require bound policy-denial
  ...
  `ReleaseStarted`; batch markers are forbidden before `ReleaseStarted` and
...
- operation-intent binding rule: mutating operations bind transaction identity
  and use the same-WAL audit barrier, while read-only operations bind query,
  snapshot, authorized plan, result label, and response-stream identity and use
  the durable audit-stream pre-release barrier,
...
- "persist" rule defining persistence as completion of the required durable
  audit barrier, not merely enqueueing an in-memory record, before protected
  bytes are handed to transport,
...
- selected and instantiated `skrifheim-engine` or `skrifheim-txn` crate for
  commit coordination, MVCC, world-head compare-and-swap, verified ingest,
  audit-coupled commit, idempotency status, and cross-crate invariants,
- instantiated `skrifheim-catalog` crate for schema roots, predicate metadata,
...
- documentation update to the workspace shape in `docs/IMPLEMENTATION_PLAN.md`
  after crate ownership is selected so the project description does not drift
  behind the actual workspace boundaries,
...
- pure deterministic merge function over `(fork base, current parent,
  candidate, schema epoch, policy epoch)` with canonical sort/traversal order
  and no dependence on hash-map iteration, wall-clock time, thread scheduling,
  callbacks, locale, filesystem order, or process-specific randomness,
- explicit conflict ordering for add, hide, supersede, invalidate, tombstone,
  legal deletion, schema changes, policy revocation, and same-valid-time
  changes,
...
- tests for divergent clones with the same `WorldId`, lost-update rejection,
  stale fork-base rejection, same-ID/different-state diff rejection, CAS head
  advancement, permutation-stable merge output, identical `WorldRevisionId`
  from identical inputs, and replay-versus-live-commit equivalence.
...
- canonical signed taint-scope creation record with engine-only/private
  constructors, idempotency key, collision rejection, no scope-ID reuse after
  retirement, and deterministic replay semantics,
- authenticated descriptor-to-scope index preventing creation of a second
  active scope with an equivalent descriptor merely to escape an existing
  invalidation fence; split and merge records are the only valid way to replace
  an existing active scope,
...
- split and merge output rule requiring every output scope to inherit every
  predecessor's fence epoch and dependency root before it can accept writes or
  prove clean dependency state,
- taint-scope tombstone garbage-collection rule allowing removal only when no
  live facts, projections, backups, rollback roots, audit proofs, legal holds,
  exports, releases, or migration proofs can reference the old scope,
- Hyve replication rule for taint scopes: assigned `TaintScopeId` values are
  carried inside replicated commands and never independently generated by each
  replica,
...
## v0.27.2 - Minimal Durable Audit Sink For Query Execution

Goal: provide the minimum encrypted durable audit sink required before
protected reads and query execution can fail closed on mandatory-audit
operations.
...
- transactional audit outbox and drainer implementing the canonical read and
  write transcript types from `v0.18.8`: persist `ReadReleasePermit` before
  read release, persist `ReleaseStarted` before handing the first protected
  byte to transport, persist `BatchMayRelease` markers where the deployment
  requires them, and bind `WriteCommitReceipt` records to WAL/manifest
  publication when the operation has a write transaction,
- high-assurance streaming rule that persists the `BatchMayRelease(sequence,
  digest)` transcript type before each protected result batch, proving the
...
- protected-read audit path for read-only operations that still require
  ...
  `ReadOutcomeReceipt` records for the explicit branches from `v0.18.8`,
  including denied-without-permit, failed-before-permit, expired-unused,
  cancelled-before-release, completed, partial, failed, disconnected, and
...
- audit precision profiles: high-assurance mode records `BatchMayRelease`
  before every protected result batch and proves the maximum batch set that may
  have escaped; standard mode may record only `ReleaseStarted` plus terminal
  outcome and may not identify the exact partial prefix after crash; neither
  mode proves client consumption,
- mandatory legal, break-glass, declassification, and high-assurance profiles
  must state whether per-batch markers are compulsory before release planning
  starts,
- audit quota reservation rule requiring enough stream quota and protected
  reserve for all required release-start and batch markers before a protected
  response starts, so a stream cannot become unauditable halfway through,
...
- maximum permitted unanchored audit record count and time window before a
  configured witness or anchor must receive the audit root,
- fail-closed behavior when the configured witness interval, receipt interval,
  audit-key availability, or audit-storage durability requirement is exceeded,
- mandatory off-node witness or external receipt policy for break-glass,
  declassification, key lifecycle, policy/law-pack changes, backup restore,
  and other critical events under high-assurance profiles,
...
- tests for protected read without durable release permit, duplicate outbox
  ...
  marker under high assurance, denied outcome without permit, expired-unused
  outcome with release started, duplicate terminal outcome, batch marker before
  release start or after terminal outcome, quota exhaustion before required
  batch markers, and stream creation/retirement racing manifest publication.
...
- full audit storage engine building on the minimal `v0.27.2` audit sink,
...
- trust-boundary statement that compiled-in extensions are trusted code and
  ...
  and that same-process panic-abort, allocator OOM, and undefined host-library
  failure behavior cannot be reliably contained by Rust crate boundaries alone,
...
- Merkleized digest-equivalence map format using canonically sorted immutable
  mapping segments, bounded segment size, streaming construction, and a global
  mapping Merkle root recorded in the migration manifest,
- inclusion proofs for individual old/new digest mappings and resumable
  validation watermarks bound to the mapping root,
- external-sort or partitioned collision detection with bounded memory, no
  duplicate old digest entries, no duplicate new digest entries, and a global
  bijection check across all mapping segments before publication,
- atomic equivalence-map publication rule: reference rewriting starts only
...
- migration-local `MigrationObjectRef` for objects whose permanent identity may
  ...
  object-format version, old suite and digest, new suite and digest,
  canonical-byte length, and optional chunk root,
- rule that `MigrationObjectRef` is usable only inside the migration engine and
  never becomes a permanent authorization capability, public object identity,
  storage isolation key, or query-visible reference; stable engine-assigned
  IDs such as `FactId` may participate but do not replace the migration-local
  reference for content-addressed blobs and roots,
- source-authentication prerequisite: the old object must be authenticated
  through the old manifest, AEAD envelope, table/segment commitment, signature,
  or relevant source root before any equivalence map is produced; recomputing
  two hashes over attacker-supplied bytes is not sufficient,
...
- secret-cleanup evidence for production providers covering unwind/error
  paths, temporary provider buffers, buffer-pool reuse, clear-on-drop coverage,
  panic/abort stance, prohibition on holding secret borrows across `await` or
  scheduler yield points, code-generation inspection for critical cleanup
  paths, and optional locked-memory/no-dump deployment profiles,
- explicit memory-secrecy non-claims stating that heap buffers can be scrubbed
...
- short-lived `PhysicalExecutionBinding` from an authorized logical plan to the
  ...
  index,
- physical binding capability rule: `PhysicalExecutionBinding` has a private
  or sealed constructor, is node-local, is not durably or publicly
  serializable except as redacted diagnostics, and binds serving node ID,
  process/storage incarnation, physical manifest generation, snapshot-lease
  ID, materialization-proof digest, projection/index compatibility keys,
  expiry, and one-execution or bounded-reuse nonce,
- immediate revalidation of `PhysicalExecutionBinding` before iterator
  creation and before every streamed output batch; failed revalidation stops
  output and records the appropriate audit outcome,
...
- leader-failover behavior for in-flight operations, idempotent command retry,
  audit/outbox preservation, freshness-anchor interaction, and committed
  manifest publication after leadership changes,
