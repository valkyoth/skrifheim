...
| WAL host-file I/O | Host-only WAL append/read helpers live outside the `no_std` core crates, append encrypted frame bodies, create Unix WAL files owner-only, read sequentially, reject unexpected domains, and detect partial headers or bodies | Scaffolded | `skrifheim-storage-host` |
...
| Quantum-aware digest policy | Configurable digest strength profiles for SHA3-256, SHA3-384, SHA3-512, SHAKE256-256, and SHAKE256-512 with full-width world, content, and manifest digest types before durable storage authority | Scaffolded | `skrifheim-crypto` |
