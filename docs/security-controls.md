...
| Sensitive equality | Security labels, policy-token sets, policy-token slots, signature envelopes, and signature sets do not expose derived `PartialEq`/`Eq`; structural comparison is explicit and not for policy or crypto decisions | Scaffolded | `scripts/validate-security-policy.sh` |
