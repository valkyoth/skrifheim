...
| Query inference | Public query plans expose aggregate proof/decision state, not per-label decisions; query requests do not expose raw result-input slices after construction, and query-result inputs do not expose raw label or sovereignty accessors publicly | Scaffolded | `skrifheim-query`, `skrifheim-policy` |
