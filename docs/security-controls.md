...
| Diagnostic redaction | Sensitive fact payloads, labels, policy tokens, signatures, query-result inputs, result classifications, planner decisions, policy proofs, query requests, and query plans use redacted `Debug`; direct sensitive `Debug` derives are release-gate blocked | Scaffolded | `scripts/validate-security-policy.sh` |
| Sensitive equality | Security labels, policy-token sets, policy-token slots, signature envelopes, signature sets, planner decisions, policy proofs, result classifications, and query plans do not expose derived `PartialEq`/`Eq`; structural comparison is explicit and not for policy or crypto decisions | Scaffolded | `scripts/validate-security-policy.sh` |
...
| Query inference | Public query plans expose aggregate proof/decision state, not per-label decisions; query requests do not expose raw result-input slices after construction | Scaffolded | `skrifheim-query` |
