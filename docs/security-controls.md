...
| Diagnostic redaction | Sensitive fact payloads, labels, policy tokens, signatures, authority contexts, query-result inputs, result classifications, planner decisions, policy proofs, query requests, query plans, query intent metadata, planner decision state, audit metadata, and exact query/proof input counts use redacted `Debug`; direct sensitive `Debug` derives are release-gate blocked | Scaffolded | `scripts/validate-security-policy.sh` |
...
| Identity and audit events | Security-relevant actions require typed actor attribution, can carry device/workload attestation evidence references, and bind event metadata to signed audit-log encryption domains | Scaffolded | `skrifheim-audit` |
