...
| Key lifecycle | Creation, activation, rotation preflight, strict epoch progression for key-material changes, planned separate event ordering for metadata-only compromise/quarantine/destruction states, unsafe-parent rejection, destruction, and destruction-gated crypto-erasure metadata | Scaffolded | `skrifheim-crypto` |
...
| WAL host-file I/O | Host-only WAL append/read helpers live outside the `no_std` core crates, append encrypted frame bodies, create and reopen Unix WAL files owner-only, fsync newly-created parent directories on Unix, reject Unix symlink WAL paths on write and read, verify CRC64-ECMA over encrypted frame bodies, read sequentially, reject unexpected domains, and detect partial headers or bodies | Scaffolded | `skrifheim-storage-host` |
