...
| WAL host-file I/O | Host-only WAL append/read helpers live outside the `no_std` core crates, append encrypted frame bodies, create and reopen Unix WAL files owner-only, reject Unix symlink WAL paths, verify CRC64-ECMA over encrypted frame bodies, read sequentially, reject unexpected domains, and detect partial headers or bodies | Scaffolded | `skrifheim-storage-host` |
