...
The project is currently at the `v0.8.0` implementation stop, pending pentest.
...
| Release notes | Active | `release-notes/RELEASE_NOTES_0.8.0.md` records scope, verification, and non-claims. |
...
| `v0.8.0` release gate | Active | `scripts/release_0_8_gate.sh` runs local checks, dependency policy, RustSec audit, CLI startup, and rootless Podman smoke. |
...
```bash
SKRIFHEIM_SKIP_PODMAN=1 scripts/release_0_8_gate.sh
```
