...
The project is currently at the `v0.3.0` implementation stop. It is not a
...
| Release notes | Active | `release-notes/RELEASE_NOTES_0.3.0.md` records scope, verification, and non-claims. |
...
| Authority-aware policy context | Scaffolded | Subject, device, and workload context constrain clearance, compartments, and releasability. |
...
| `v0.3.0` release gate | Active | `scripts/release_0_3_gate.sh` runs local checks, dependency policy, RustSec audit, CLI startup, and rootless Podman smoke. |
...
```bash
SKRIFHEIM_SKIP_PODMAN=1 scripts/release_0_3_gate.sh
```
